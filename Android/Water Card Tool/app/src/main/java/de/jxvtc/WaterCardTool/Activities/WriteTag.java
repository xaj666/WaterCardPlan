...
import de.jxvtc.WaterCardTool.MCReader;
...
public class WriteTag extends BasicActivity {
    ...
    public final static String EXTRA_DUMP =
        "de.jxvtc.WaterCardTool.Activity.DUMP";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        radioGroup.setOnCheckedChangeListener((group, checkedId) -> {
            if (checkedId == R.id.radioButton1) {
                XAJ = "E80300000000150C0D1623160C000074";
                Log.d("RadioButtonCheck", "10块钱被选中");
                Money = 10;
            } else if (checkedId == R.id.radioButton2) {
                XAJ = "B80B00000000150C0D1623160C00004C";
                Log.d("RadioButtonCheck", "30块钱被选中");
                Money = 30;
            } else if (checkedId == R.id.radioButton3) {
                XAJ = "881300000000150C0D1623160C000024";
                Log.d("RadioButtonCheck", "50块钱被选中");
                Money = 50;
            } else if (checkedId == R.id.radioButton4) {
                XAJ = "102700000000150C0D1623160C0000C0";
                Log.d("RadioButtonCheck", "100块钱被选中");
                Money = 100;
            }
        });
            ...
            case CKM_WRITE_BLOCK:
                if (resultCode != Activity.RESULT_OK) {
                    ...
                    if(MoneyFlag) {
                        ...
                        Toast.makeText(getApplicationContext(), "错误:余额不足，请充值", Toast.LENGTH_SHORT).show();
                    }
    ...
    public void onWriteBlock(View view) {
        ...
        final int sector = Integer.parseInt(
            mSectorTextBlock.getText().toString());//扇区
        final int block = Integer.parseInt(
            mBlockTextBlock.getText().toString());//块地址
    ...
    private void writeBlock() {
            ...
            // 第一次写块
            int block1 = 1;
            if (sector == 0 && block1 == 0) {
                block1 = 1; // Ensure we write to block 1 for manufacturer block
            }
            result1 = writeBlock(reader, sector, block1, data, keys);

            // 第二次写块
            int block2 = 2;
            result2 = writeBlock(reader, sector, block2, data, keys);
    ...
    private int writeBlock(MCReader reader, int sector, int block, String data, byte[][] keys) {
        int result = -1;
        if (keys[1] != null) {
    ...
    private void fetchScoreSync() {
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    URL url = new URL(Login.API_Score);
                    ...
                    if (reader != null) {
                        try {
                            ...
                            Log.e("XAJ", "Error closing stream", e);
                        }
                    }
                }
                ...
                if (responseData != null) {
                    try {
                        ...
                        Log.e("XAJ", "Error parsing JSON", e);
                    }
                }
                ...
                if (AndroidArray != null && AndroidArray.length > 0) {
                    ...
                    //Log.d(TAG, "score: \n" + keys);
                    Login.SeverScore = keys;
                } else {
                    // 处理未获取到数据的情况
                    Toast.makeText(WriteTag.this, "服务器错误：未获取到积分数据", Toast.LENGTH_LONG).show();
                }
            }
        });
